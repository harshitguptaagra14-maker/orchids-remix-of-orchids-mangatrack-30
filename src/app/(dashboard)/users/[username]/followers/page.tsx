...
export default function FollowersPage({ params }: { params: Promise<{ username: string }> }) {
  ...
  const fetchFollowers = useCallback(async (page = 1, append = false) => {
    ...
    try {
      const res = await fetch(`/api/users/${username}/followers?page=${page}&limit=20`)
      ...
      if (res.ok) {
        ...
        users.forEach((u: User) => {
          if (u.isFollowing) newFollowing.add(u.id)
        })
        setFollowingIds(newFollowing)
      } else if (res.status === 404) {
        router.push("/404")
      } else if (res.status === 403) {
        setError("This user's followers list is private")
      } else {
  ...
  const handleFollow = useCallback(async (userId: string, isCurrentlyFollowing: boolean) => {
    ...
    const targetUser = followers.find(u => u.id === userId)
      ...
      <div className="p-6 space-y-8 max-w-3xl mx-auto pb-24">
        ...
        <div className="flex flex-col items-center justify-center py-24 text-center space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Button onClick={() => fetchFollowers(1)} variant="outline" className="rounded-full">
              Try again
            </Button>
          </div>
        </div>
  ...
  return (
    <div className="p-6 space-y-8 max-w-3xl mx-auto pb-24">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-50">Followers</h1>
